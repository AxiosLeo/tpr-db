 ...
 * @blog:  http://hanxv.cn
 * @datetime: 2018/4/10 16:22
 */

namespace tpr\db\query;
...
use tpr\db\builder\MongoBuilder;
use tpr\db\connector\MongoConnector;
use tpr\db\core\Parse;
use tpr\db\Db;
use tpr\db\exception\DataNotFoundException;
...
// 数据库Connection对象实例
    /**
     * @var \tpr\db\connector\MongoConnector
     */
    protected $connection;
    // 数据库Builder对象实例
    /**
     * @var \tpr\db\builder\MongoBuilder
     */
    protected $builder;
    // 当前模型类名称
    protected $model;
     ...
     * 架构函数
     * @access public
     * @param \tpr\db\connector\MongoConnector    $connection 数据库对象实例
     */
    public function __construct(MongoConnector $connection = null)
    {
    ...
    public function __call($method, $args)
    {
        if (strtolower(substr($method, 0, 5)) == 'getby') {
            // 根据某个字段获取记录
            $field         = Parse::parseName(substr($method, 5));
            $where[$field] = $args[0];
            return $this->where($where)->find();
        } elseif (strtolower(substr($method, 0, 10)) == 'getfieldby') {
            // 根据某个字段获取记录的某个值
            $name         = Parse::parseName(substr($method, 10));
            $where[$name] = $args[0];
     ...
     * @access public
     * @return MongoConnector|\tpr\db\core\Connection
     */
    ...
    public function getTable($name = '')
    {
        if ($name || empty($this->table)) {
            ...
            if ($name) {
                $tableName .= Parse::parseName($name);
            }
        } else {
     ...
     * @return bool|int|\MongoDB\Driver\WriteResult
     */
     ...
     * @return int|null|string|WriteResult
     * @throws Exception
     */
     ...
     * @access public
     * @param mixed $data 数据
     * @return int|WriteResult|null|string
     * @throws Exception
     */
     ...
     * 查找记录
     * @access public
     * @param array|string|MongoQuery|\Closure $data
     * @return bool|mixed
     * @throws Exception
     */
    public function select($data = null)
    {
        if ($data instanceof MongoQuery) {
            return $data->select();
     ...
     * 查找单条记录
     * @access public
     * @param null|MongoQuery|\Closure $data
     * @return bool|null|MongoQuery|Cursor
     * @throws Exception
     */
