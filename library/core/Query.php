...
class Query
{
     ...
     * @return array|false|int|mixed|null|\PDOStatement|string
     ...
     * @return int|mixed|null|\PDOStatement|string
     * @throws BindParamException
     * @throws Exception
     * @throws PDOException
     * @throws \ErrorException
    ...
    public function withCount($relation, $subQuery = true)
    {
        if (!$subQuery) {
            ...
            foreach ($relations as $key => $relation) {
                ...
                $relation = Parse::parseName($relation, 1, false);
                $count    = '(' . (new $this->model)->$relation()->getRelationCountQuery($closure) . ')';
                $this->field([$count => Parse::parseName($relation) . '_count']);
            }
        }
        return $this;
    }
     ...
     */
    private function throwNotFound($options = [])
    {
        $table = is_array($options['table']) ? key($options['table']) : $options['table'];
