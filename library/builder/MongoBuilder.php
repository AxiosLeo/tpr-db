...
class MongoBuilder
{
     ...
     * 架构函数
     * @access public
     * @param MongoConnector $connection 数据库连接对象实例
     * @param MongoQuery $query 数据库查询对象实例
     */
    public function __construct(MongoConnector $connection, MongoQuery $query)
    {
        ...
        foreach ($where as $logic => $val) {
            foreach ($val as $field => $value) {
                if ($value instanceof \Closure) {
                    // 使用闭包查询
                    $query = new MongoQuery($this->connection);
